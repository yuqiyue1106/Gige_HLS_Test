...
// ==============================================================
`timescale 1ns/1ps
module brenner_accel_ctrl_s_axi
#(parameter
    ...
    input  wire [63:0]                   brenner_score,
    input  wire                          brenner_score_ap_vld,
...
localparam
    ...
    ADDR_IER                  = 6'h08,
    ADDR_ISR                  = 6'h0c,
    ADDR_BRENNER_SCORE_DATA_0 = 6'h10,
    ADDR_BRENNER_SCORE_DATA_1 = 6'h14,
    ADDR_BRENNER_SCORE_CTRL   = 6'h18,
...
//------------------------Local signal-------------------
    ...
    reg                           int_brenner_score_ap_vld;
    reg  [63:0]                   int_brenner_score = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BRENNER_SCORE_DATA_0: begin
                    rdata <= int_brenner_score[31:0];
                end
                ADDR_BRENNER_SCORE_DATA_1: begin
                    rdata <= int_brenner_score[63:32];
                end
                ADDR_BRENNER_SCORE_CTRL: begin
                    rdata[0] <= int_brenner_score_ap_vld;
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (brenner_score_ap_vld)
            int_brenner_score <= brenner_score;
    end
end

// int_brenner_score_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_brenner_score_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (brenner_score_ap_vld)
            int_brenner_score_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_BRENNER_SCORE_CTRL)
            int_brenner_score_ap_vld <= 1'b0; // clear on read
    end
end
